...
def simulate_queue(arrival_interval_min, arrival_interval_max, service_interval_min, service_interval_max, num_servers, queue_capacity, first_arrival_time):
    ...
    # 0 is the type of arrival event
    heapq.heappush(event_heap, (first_arrival_time, 0, current_customer))
    ...
    while count > 0 and event_heap:
        ...
        # Accumulate time before processing the event
        ...
        if event_type == 0:  # Arrival
            if len(queue) < queue_capacity:
                ...
                if len(queue) <= num_servers:
                    ...
                    # 1 is the type of departure event
                    heapq.heappush(
                        event_heap, (departure_time, 1, current_customer))
            else:
                lost_customers += 1
            current_customer += 1
            next_arrival_time = event_time + generate_arrival_time()
            heapq.heappush(
                event_heap, (next_arrival_time, 0, current_customer))
            ...
            if queue and len(queue) >= num_servers:
                ...
                heapq.heappush(
                    event_heap, (next_departure_time, 1, next_customer))
    ...
    for i in range(queue_capacity + 1):
        ...
        print(f"State {i}: Accumulated time {
              accumulated_times[i]:.4f}, Probability {probability:.2f}%")

    print(f"Lost customers: {lost_customers}")
    print(f"Total simulation time: {last_event_time:.4f}\n")
